...
const groupMocksByMethodAndStatus = (
	globalMocks: Mock[],
): Map<string, Map<string, Mock[]>> => {
...
export const globalMockUrql = (
	...
	groupedMocks.forEach((methodStatusMap, apiUrl) => {
		methodStatusMap.forEach((mocks, key) => {
			...
			const status =
				config.status ||
				(defaultStatusStr ? Number.parseInt(defaultStatusStr, 10) : 200);
			...
			mockData.push({
				...
				response: (request: MockRequest) => {
					for (const mock of mocks) {
						const mockOperationName = getOperationNameFromQuery(
							mock.request.query,
						);
...
export type MockRequest = {
	body: any; // TODO - replace `any` with a type of response.  I think it is just string from mock util.
	method: RequestMethod;
	url: string;
	signal?: any; // TODO - replace `any` with the proper type
